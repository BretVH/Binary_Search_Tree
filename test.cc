...
int main( int argc, char *argv[] )
{
  ...
  int n = 1364;  // <-- try changing the value of 'n'
...
/*
  ...
  // Check equality
  if (!(tree_copy == tree)) {
    cerr << "== operator: expected true, got false 1\n";
  }
  if (tree_copy != tree) {
  ...
  if (!(tree_copy == tree)) {
    cerr << "== operator: expected true, got false 2\n";
  }
  ...
  cout << "Preorder debug tree:\n";
  tree.preorder(func);
  cout << "\n";
  // Check the traversals
  cout << "Preorder traversal debug tree2:\n";
  tree2.preorder(func);
  cout << "\n";
  BinaryTree<int> tree3(tree); 
  // Check the traversals
  cout << "Preorder traversal debug tree3:\n";
  tree3.preorder(func);
  cout << "\n";


  if (!(tree2 == tree)) {
    cerr << "== operator: expected true, got false 3\n";
  }
...
*/
  ...
  //BTNode<int>* myNode(tree.get_parent( tree.root->right->right ));
  //cout << myNode->elem << " should equal : " << tree.root->right->elem << endl;
}
